...
This repository provides code to develop and evaluate a framework of image registration informed 3D-UNets to automatically localize the Anterior (AC) and Posterior Commissure (PC) landmarks on non-contrast Computed Tomography (CT) scans. While the codebase is for axial reformats, it can be readily extended to coronal and sagittal reformat-based localization. 
...
│   └── patient_002.nii
│
├── ct_template/                                     # INPUT: Reference atlas files. Download the atlase files and place them here. See ct_template/TEMPLATE_README for details
│   ├── miplab_to_mni_sym_warp.nii.gz
│   └── miplab-ncct_sym_brain.nii.gz
│
├── acpc_annotations/             
│   ├── acpc_screenshots/                            # OUTPUT: Generated by processes in CoarseLocalizationSlicer.ipnyb
│   ├── acpc_coarse.csv                              # OUTPUT: Generated by processes in CoarseLocalizationSlicer.ipnyb
│   └── acpc_gt.csv                                  # INPUT: 'acpc_gt.csv' would contain reference standard AC-PC landmarks 
│
├── brain_vols/                                      # OUTPUT: Generated by CoarseLocalizationSlicer.ipynb (skull stripped head CT)
...
│   ├── ip_patches/
│   │   └── patient_001/
│   │       ├── input_patches_Rot_0.npy              # Represents original patched scan without any rotations
│   │       ├── ...
│   │       └── input_patches_Rot_49.npy             # Assuming that there are 48 rotations per scan, this represents the rotated and patched scan
│   └── op_patches/
│       └── patient_001/
│           ├── hm_patches_Rot_0.npy                 # Represents original patched heatmap without any rotations
│           ├── ...
│           └── hm_patches_Rot_49.npy                # Assuming that there are 48 rotations per scan, this represents the rotated and patched heatmap
│
...
│   ├── hp_trial_results.csv                         # Main record required for selecting the optimal hyperparameters
│   ├── tracked_train_set_predictions_outer.csv      # Predicted AC-PC coordinates for scans in the training sets, across the 5 outer folds
│   ├── tracked_test_set_predictions.csv             # Predicted AC-PC coordinates for scans in the testing sets, across the 5 outer folds
│   ├── tracked_test_set_metrics.csv                 # Error metrics (MRE, absolute coordinate-wise errors) for scans in the testing sets, across the 5 outer folds 
│   ├── tracked_epoch_level_metrics.csv              # Epoch-wise loss and AC/PC MREs, absolute coordinate-specific errors for each outer fold (x5), nested inner fold (x4), and hyperparameter combination (x25)
│   ├── tracked_train_loss_outer.csv                 # Epoch-wise loss, AC/PC MREs, absolute coordinate-specific errors for each outer fold (x5), based on the model trained on the entire training set using the optimal HPs from nested CV
│   ├── scan_id_map.csv                              # Mapping between scan identifier and its index in the long tensors containining dataset-wide i/p and o/p patches
│   ├── inner_fold_val_metrics.csv                   # For each hp config, outer fold, and nested CV fold, this tracks the best error metrics (MRE, absolute coordinate-specific errors)
│   ├── best_model_outer_fold 0.pt                   # Best model (characterized by optimal hyperparameters) chosen in the particular outer fold of the 5-fold CV
│   ├── ...
│   ├── best_model_outer_fold 4.pt
│   └── HyperParametersSelected.csv                  # Record of the hyperparameter combinations selected by random search. These will be evaluated with nested CV
│
└── unet_results_aug/                                # OUTPUT: Generated by trainWaug_test.py, contains results of training models at increasing augmentation levels
    ├── AugmentedTimes2/
    │   └── augmentations_selected_times_2.csv       # Contains the random augmentation (rotation) ids selected for each scan, for each augmentation scale (times_2 represents 2x rotations)
    │   └── tracked_test_set_predictions.csv         # Predicted AC-PC coordinates for scans in the testing sets, across the 5 outer folds
    │   └── tracked_test_set_metrics.csv             # Error metrics (MRE, absolute coordinate-wise errors) for scans in the testing sets, across the 5 outer folds
    │   └── tracked_train_set_metrics_outer.csv      # For each outer fold and each (augmented) training scan, this contains epoch-level error metrics for the AC-PC predictions by the model
    │   └── tracked_train_set_predictions_outer.csv  # For each outer fold and each (augmented) training scan, this contains AC-PC predictions by the model for the last epoch
    │   └── scan_id_map.csv                          # Mapping between scan identifier and its index in the long tensors containining dataset-wide i/p and o/p patches
    ├── ...
    └── AugmentedTimes20/          
``` 
