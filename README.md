...
│   ├── tracked_train_set_metrics_outer.csv  # Predicted AC-PC coordinates for scans in the training sets, across the 5 outer folds. (rename to predictions)
...
│   ├── tracked_metrics.csv                  # Epoch-wise loss and AC/PC MREs, absolute coordinate-specific errors for each outer fold (x5), nested inner fold (x4), and hyperparameter combination (x25)
│   ├── track_train_loss_outer.csv           # Epoch-wise loss, AC/PC MREs, absolute coordinate-specific errors for each outer fold (x5), based on the model trained on the entire training set using the optimal HPs from nested CV.
