...
def train_model(config, data_dict = None):
    ...
    for train, valid in inner_gkf.split(data_df): 
        ...
        #After all epochs, for this split, what was the best validation MRE? This is almost always the last epoch.  
        val_metrics_dict = {"loss": validation_loss_epochs[best_epoch], 
        ...
        val_metrics.to_csv(inner_fold_val_metrics_file_path, mode='a', header=not os.path.exists(inner_fold_val_metrics_file_path)) #best metrics across all epochs for the 4 folds
        ...
        exp_metrics_file_path = os.path.join(results_folder, 'tracked_metrics.csv') #epoch level metrics for each fold
        exp_metrics.to_csv(exp_metrics_file_path, mode='a', header=not os.path.exists(exp_metrics_file_path))
        #best epoch's metrics for each fold
...
def test_accuracy(outer_fold, train_valid_dataloader, test_dataloader, unet_model, criterion, optimizer, exp_lr_scheduler, num_epochs, voxel_center_offset, results_folder, device):
    ...
    track_train_set_predictions_path_outer = os.path.join(results_folder, 'tracked_train_set_metrics_outer.csv')
    track_train_loss_path_outer =  os.path.join(results_folder, 'track_train_loss_outer.csv')
    best_mre = np.inf
    for epoch in range(num_epochs):
        print(f'Epoch {epoch}/{num_epochs - 1}')
        print('-' * 10)
...
def main(data_dir, num_samples, num_epochs, patch_size, min_thresh, hp_search_space, batch_size):
    ...
    for split in outer_skf.split(scan_info_df_0):
        ...
        for i in range(len(hp_samples_df)):
            ...
            cur_trial_results_file_path = os.path.join(results_dir, 'trial_results.csv')
            cur_trial_results_df.to_csv(cur_trial_results_file_path, mode='a', header=not os.path.exists(cur_trial_results_file_path))
        ...
        criterion = SigmoidFocalLoss(best_alpha, best_gamma, best_min_thresh, "mean")
